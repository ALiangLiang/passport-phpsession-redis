...
var passport = require('passport-strategy')
  , util = require('util')
  , redis = require('redis')
  , unserialize = require('node-php-session-unserialize')
...
Strategy.prototype.authenticate = function(req) {
  ...
  this._redisClient.get(sessionId, function (err, result) {
    ...
    // Unserialize PHP session in redis.
    var sessionObj = unserialize(result)
    var session = {}
    // Get the session we want. And merge them into one object.
    sessionObj[sessionId].forEach((phpArrayElement) => Object.assign(session, phpArrayElement))
